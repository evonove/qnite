...
QniteAxes::QniteAxes(QQuickItem *parent)
    : QQuickItem(parent), m_lowerXBound{0}, m_upperXBound{0}, m_xPadding{0},
      m_lowerYBound{0}, m_upperYBound{0}, m_yPadding{0},
      m_canvas{new QQuickItem}, m_axisX{nullptr}, m_axisY{nullptr} {
  m_canvas->setParentItem(this);
  m_canvas->setClip(true);

  connect(this, &QQuickItem::widthChanged,
          [=]() { this->m_canvas->setWidth(width()); });
  connect(this, &QQuickItem::heightChanged,
          [=]() { this->m_canvas->setHeight(height()); });
}
...
QQuickItem *QniteAxes::canvas() const { return m_canvas; }

QQmlListProperty<QniteArtist> QniteAxes::artists() {
  return QQmlListProperty<QniteArtist>(
      this, nullptr, &QniteAxes::append_artists, &QniteAxes::count_artists,
      &QniteAxes::at_artists, &QniteAxes::clear_artists);
}

QQmlListProperty<QniteTool> QniteAxes::tools() {
  return QQmlListProperty<QniteTool>(
      this, nullptr, &QniteAxes::append_tools, &QniteAxes::count_tools,
      &QniteAxes::at_tools, &QniteAxes::clear_tools);
}
...
QniteAxis *QniteAxes::axisY() const { return m_axisY; }
...
QniteAxis *QniteAxes::axisX() const { return m_axisX; }
...
void QniteAxes::append_artists(QQmlListProperty<QniteArtist> *property,
                               QniteArtist *value) {
...
void QniteAxes::append_tools(QQmlListProperty<QniteTool> *property,
                             QniteTool *value) {
  QniteAxes *axes = qobject_cast<QniteAxes *>(property->object);
  value->setAxes(axes);
  value->setParentItem(axes->canvas()); // TODO: move down into artist?
